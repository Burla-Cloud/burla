...
- **Nodes have no cloud API access.** Only the head creates and deletes VMs. A node that must die (inactivity, boot failure) pushes its terminal status then calls `POST /v1/nodes/{id}/self_delete` and the head deletes the VM.
