...
- `make local-dev` nukes `_shared_workspace/`, `_node_auth/`, and `_local_dev_state/` on startup for a clean slate. `_image_seed/` survives; it holds the `docker save`d default worker image each node's inner daemon loads at boot. Anything expensive a node builds for itself lives in a `burla-{cluster}-*` docker volume instead: the inner image store and the shared worker python env (both per node port, so a replacement node inherits its predecessor's) and the uv cache (one per cluster). Only `make stop` reclaims those.
