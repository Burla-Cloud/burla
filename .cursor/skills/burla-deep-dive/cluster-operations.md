...
- Nodes are privileged Docker containers acting as fake VMs, not real VMs (`LocalDockerProvider`). Each runs its own docker daemon (started by `node_service/local_dev_entrypoint.sh`) with its workers inside it, so `node_service`'s worker code is identical to prod. Ports auto-increment starting from `LOCAL_DEV_NODE_PORT_BASE` (`_current_local_dev_max_node_port`, computed by scanning the `host` fields of active nodes in `cluster_state`).
...
- `make local-dev` nukes `_shared_workspace/`, `_node_auth/`, and `_local_dev_state/` on startup for a clean slate (`/worker_service_python_env` is node-local now, wiped with the node container). `_image_seed/` and `_uv_cache/` survive: the first holds the `docker save`d default worker image each node's inner daemon loads at boot, the second is the uv cache shared by the node's own env sync and every worker's env install.
