...
Same pre-write (`cluster_shutdown: True` on running jobs), then `_shutdown_cluster` synchronously: takes every node in `cluster_state.list_nodes()` with status in `[READY, BOOTING, RUNNING]`, constructs a `Node.from_state` for each, calls `node.delete()` in a thread pool. In local-dev, it also removes leftover containers labeled `burla-cluster-member={cluster}` (just node containers; workers live inside each node's own docker daemon and die with it). Their inner-docker volumes are deliberately left behind for the next node in that port slot.
...
2. Create the instance through the compute provider ([providers/](../../../main_service/src/main_service/providers/), selected by `get_provider()` from `CLOUD_PROVIDER` / `IN_LOCAL_DEV_MODE`):
   ...
   - **AWS** (`aws.py`): EC2 twin. The node AMI is looked up by the `burla-node-image=true` tag (built by `burla deploy --cloud=aws`), AZs are iterated on capacity errors, GPU nodes are not supported yet.
   - **local-dev** (`local_docker.py`): a privileged Docker container acting as a fake VM on that cluster's own network (`burla-{cluster name}`, namespaced per checkout so several local-dev clusters coexist). Its entrypoint (`node_service/local_dev_entrypoint.sh`) puts itself in a `burla-node-service.slice` cgroup at `cpu.weight=1000` and pre-creates `burla-workers.slice` at `80` (the weights a real VM's systemd units get), starts an inner docker daemon on a named volume at `/var/lib/docker`, loads the `_image_seed/` tarball unless that image is already in the store, and resolves the head's `host.docker.internal` alias to an IPv4 so inner workers can reach it. The volume is named `burla-{cluster}-docker-{port}`, so a replacement node in the same port slot reuses the previous node's image store instead of unpacking ~1GB again. `node_service/` is volume-mounted so code reloads on save; hostname is `http://node_{suffix}:{port}`, and the published host port comes from the cluster's `LOCAL_DEV_NODE_PORT_BASE`.
...
- Nodes are privileged Docker containers acting as fake VMs, not real VMs (`LocalDockerProvider`). Each runs its own docker daemon (started by `node_service/local_dev_entrypoint.sh`) with its workers inside it, so `node_service`'s worker code is identical to prod. Each node takes the lowest free port above `LOCAL_DEV_NODE_PORT_BASE` (`_taken_local_dev_node_ports`, the union of active nodes' `host` ports in `cluster_state` and ports still bound by this cluster's live containers), so a replacement node reuses the slot, and the port-keyed caches, of the node it replaces.
- The head is **not** a container: `make local-dev` runs it as a host subprocess (`python -m burla._local_dev`) so it uses this checkout's code directly. Nodes reach it at `host.docker.internal:<head port>` (resolved to an IPv4 by the node entrypoint so inner workers can reach it too), and the head reaches nodes at `127.0.0.1:<published node port>` rather than by container name.
...
- `make local-dev` nukes `_shared_workspace/`, `_node_auth/`, and `_local_dev_state/` on startup for a clean slate. `_image_seed/` survives; it holds the `docker save`d default worker image each node's inner daemon loads at boot. Anything expensive a node builds for itself lives in a docker volume instead: the inner image store and the shared worker python env (`burla-{cluster}-*`, per node port, so a replacement node inherits its predecessor's) and the uv cache (`burla-uv-cache`, one per machine shared by every cluster). `make stop-all` reclaims all of these caches.
- `make local-dev` rebuilds the node base image when `node_service/Dockerfile` has changed since the tagged image was built (compared through a `burla.node-image-fingerprint` label). New node_service *dependencies* are invisible to that check, since they come from a git clone rather than this checkout: run `make local-images` by hand for those.
- The head's SQLite history db lives on the host at `_local_dev_state/history.db` (via `HISTORY_DB_PATH`), so it is reset on each `make local-dev`. The head removes its node containers; workers die with their node.
...
- The inactivity watchdog does not run in local-dev; the head removes nodes through its normal shutdown and restart paths, and the next `make local-dev` start sweeps stale containers.
