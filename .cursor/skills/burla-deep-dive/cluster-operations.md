...
Same pre-write (`cluster_shutdown: True` on running jobs), then `_shutdown_cluster` synchronously: takes every node in `cluster_state.list_nodes()` with status in `[READY, BOOTING, RUNNING]`, constructs a `Node.from_state` for each, calls `node.delete()` in a thread pool. In local-dev, also `docker rm -f -v` any leftover containers labeled `burla-cluster-member={cluster}` (just node containers; workers live inside each node's own docker daemon and die with it).
...
2. Create the instance through the compute provider ([providers/](../../../main_service/src/main_service/providers/), selected by `get_provider()` from `CLOUD_PROVIDER` / `IN_LOCAL_DEV_MODE`):
   ...
   - **local-dev** (`local_docker.py`): a privileged Docker container acting as a fake VM on that cluster's own network (`burla-{cluster name}`, namespaced per checkout so several local-dev clusters coexist). Its entrypoint (`node_service/local_dev_entrypoint.sh`) starts an inner docker daemon (anonymous volume at `/var/lib/docker`), loads the `_image_seed/` tarball, and resolves the head's `host.docker.internal` alias to an IPv4 so inner workers can reach it. `node_service/` is volume-mounted so code reloads on save; hostname is `http://node_{suffix}:{port}`, and the published host port comes from the cluster's `LOCAL_DEV_NODE_PORT_BASE`.
...
- `make local-dev` nukes `_shared_workspace/`, `_node_auth/`, and `_local_dev_state/` on startup for a clean slate (`/worker_service_python_env` is node-local now, wiped with the node container). `_image_seed/` survives: it holds the `docker save`d default worker image that each node's inner daemon loads at boot.
- The head's SQLite history db lives on the host at `_local_dev_state/history.db` (via `HISTORY_DB_PATH`), so it is reset on each `make local-dev`. `make stop` `docker rm -f -v`s this cluster's labeled node containers (workers and the inner image store die with them); there is no external state to clean up.
