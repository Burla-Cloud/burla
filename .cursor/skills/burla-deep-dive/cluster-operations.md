...
- `make local-dev` nukes `_shared_workspace/`, `_node_auth/`, and `_local_dev_state/` on startup for a clean slate. `_image_seed/` survives; it holds the `docker save`d default worker image each node's inner daemon loads at boot. Anything expensive a node builds for itself lives in a docker volume instead: the inner image store and the shared worker python env (`burla-{cluster}-*`, per node port, so a replacement node inherits its predecessor's; reclaimed by `make stop`) and the uv cache (`burla-uv-cache`, one per machine shared by every cluster; survives `make stop` because a cold cache makes the next run's env installs heavy enough to wedge nodes, reclaimed by `make stop-all`).
...
- The head's SQLite history db lives on the host at `_local_dev_state/history.db` (via `HISTORY_DB_PATH`), so it is reset on each `make local-dev`. `make stop` `docker rm -f`s this cluster's labeled node containers (workers die with them) and removes its labeled volumes (the nodes' inner image stores); there is no external state to clean up.
