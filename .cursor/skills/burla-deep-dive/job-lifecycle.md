...
5. Detects completion: when `all_inputs_uploaded` is true, the local inputs queue (including pending transfers) is empty, and all workers are idle, it pushes progress once more and completes if `client_has_all_results` is true, or, when the client is disconnected/detached, if `total_num_results == n_inputs` in the returned view. A node may leave earlier only after it has explicitly traded away every slot (`target_parallelism <= 0`) and drained its local inputs, workers, transfers, and acknowledged results. On completion/failure/cancel it cancels the steal task, PATCHes the job status (`COMPLETED` unless the view already says FAILED/CANCELED; the head ignores a COMPLETED write unless the job is still RUNNING), and runs `reset_workers` → `reinit_node`.
...
- An empty neighbor is only a momentary observation, not proof of global completion: that neighbor can receive requeued or stolen work later. It slows the next steal attempt but never lets a positive-capacity node leave the job.
...
1. Client PATCHes `jobs/{id}` via main_service with `client_has_all_results: True` and `status: "COMPLETED"`. The head preserves whichever terminal status was recorded first, so this cannot overwrite a concurrent cancellation or failure.
2. Client cancels all `node_tasks`.
3. Each node's `job_watcher` sees `client_has_all_results` in the job view on its next push (within ~1s), cancels the steal task, PATCHes `jobs/{id}.status = "COMPLETED"`, and asks the head to detach it from the job before maintenance. Healthy worker containers are reset in place, only missing or broken capacity is recreated, surplus slot-trade workers are removed, and the node then pushes `READY`.
4. A maintenance failure marks and deletes that node but cannot retroactively fail the detached job. The node keeps its last-known-good authorized-user cache through maintenance; backend refresh uses bounded retries and is not required when cached credentials exist.

If a node flips to `FAILED` mid-job, the client raises `NodeDisconnected`. If a node's heartbeat is lost, the client still catches the exception and asks main_service via `GET /v1/jobs/{id}` whether a lifecycle signal (`cluster_shutdown`, `cluster_restarted`, `dashboard_canceled`) was set; if so, it raises the matching domain exception instead of the bare network error. A `/results` poll that receives HTTP 500, 502, 503, or 504 retries with short backoff for at most 10 seconds while checking those lifecycle signals.
