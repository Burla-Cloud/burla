...
4. Run `scripts/dev_vm_create.sh`; it chooses a slot, starts or creates the VM, and prints the slot state.
...
8. For Burla CLI auth, use the VM shell and run `uv run --project ./client --group dev burla login --no_browser=True`; open the printed URL in the same GStack browser session.
...
- Never assume the dashboard URL is `http://localhost:5001`; use the URL printed by `scripts/dev_vm_create.sh`.
