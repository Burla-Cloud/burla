...
`tests/scenarios/test_node_silent_but_alive.py` raises its own timeout: it
freezes a node for a minute, and runs a workload that pins a node for several,
to prove neither is mistaken for a dead node.
