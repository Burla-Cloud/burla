...
def test_BURLA_CLUSTER_DASHBOARD_URL_overrides_config(monkeypatch, tmp_path):
    ...
    config.write_text(json.dumps({"cluster_dashboard_url": "https://from-config"}))
    monkeypatch.setattr(burla, "CONFIG_PATH", config)

    monkeypatch.setenv("BURLA_CLUSTER_DASHBOARD_URL", "https://env-wins")
    assert get_cluster_dashboard_url() == "https://env-wins"
