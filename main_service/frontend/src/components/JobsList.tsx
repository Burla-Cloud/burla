...
// } from "@/components/ui/table";
// import { cn } from "@/lib/utils";
// import { Square } from "lucide-react"; // Import icons
// import { useJobs } from "@/contexts/JobsContext"; // ✅ Import useJobs from context
// import { BurlaJob } from "@/types/cluster";

// export const JobsList = () => {
//     const { jobs, setJobs, selectedFilter } = useJobs(); // ✅ Get selectedFilter from context
//     const anySelected = jobs.some((job) => job.checked);

//     // ✅ Helper to get the cutoff timestamp based on selectedFilter
//     const getCutoffTime = (): number => {
//         const now = new Date();
...
//     // ✅ Filter jobs based on `selectedFilter`
//     const filteredJobs = jobs.filter((job) => {
//         return job.started_at && job.started_at.getTime() >= getCutoffTime();
//     });
...
//     const handleSelectAllChange = () => {
//         const newSelectAll = !filteredJobs.every((job) => job.checked);
//         setJobs((prevJobs) =>
//             prevJobs.map((job) => ({ ...job, checked: newSelectAll }))
//         );
//     };
...
//                                         <input
//                                             type="checkbox"
//                                             checked={filteredJobs.every((job) => job.checked)}
//                                             onChange={handleSelectAllChange}
//                                             className="w-4 h-4 border-2 border-gray-400 rounded-none appearance-none checked:bg-[#3b5a64] checked:border-[#3b5a64] cursor-pointer"
//                                         />
//                                     </TableHead>
...
//                                             <div className="relative group">
//                                                 <button 
//                                                     className={`text-[#3b5a64] hover:opacity-75 transition-opacity ${
//                                                         anySelected ? "opacity-100" : "opacity-0 pointer-events-none"
//                                                     }`}
//                                                 >
//                                                     <Square size={23} />
//                                                 </button>
...
export const JobsList = () => {
    ...
    // ✅ Filter and sort jobs based on `selectedFilter`
    const filteredJobs = useMemo(() => {
        return jobs
            .filter((job) => job.started_at && job.started_at.getTime() >= getCutoffTime)
            .sort((a, b) => (b.started_at?.getTime() || 0) - (a.started_at?.getTime() || 0)); // Sort newest first
    }, [jobs, getCutoffTime]);
