...
// export const NodesList: React.FC<NodesListProps> = ({ nodes }) => {
...
//     1,
//     Math.ceil(activeNodes.length / PAGE_SIZE)
//   );
...
//         const total: number =
//           typeof json.total === "number" ? json.total : mapped.length;
...
//   const noDeletedNodes =
//     showDeleted && !deletedLoading && deletedNodes.length === 0;
...
//             // same spinner style as JobsList
...
export const NodesList: React.FC<NodesListProps> = ({ nodes, showDeleted }) => {
  const [showWelcome, setShowWelcome] = useState(true);
  const [copied, setCopied] = useState(false);
