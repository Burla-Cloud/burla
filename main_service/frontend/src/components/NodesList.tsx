...
remote_parallel_map(my_function, list(range(1000)))`;
  ...
  const sortedNodes = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      if (sortKey === "status") {
        av = STATUS_RANK[String(a.status)] ?? 999;
        bv = STATUS_RANK[String(b.status)] ?? 999;
      } else {
        av = a.name;
        bv = b.name;
      }
  ...
  // Only animate rows on first mount (not on sort)
  const didMountRef = useRef(false);
  useEffect(() => {
    didMountRef.current = true;
  }, []);

  // Fixed-size sort icon box; fades between states (no layout shift)
    ...
    <span className="relative inline-flex items-center justify-center w-3.5 h-3.5 shrink-0">
      <ChevronUp
        className={cn(
          ...
          active && sortDir === "asc" ? "opacity-100" : "opacity-0"
        )}
      />
      <ChevronDown
        className={cn(
          "absolute h-3.5 w-3.5 transition-opacity",
          active && sortDir === "desc" ? "opacity-100" : "opacity-0"
        )}
      />
      <ArrowUpDown
        className={cn(
          "absolute h-3.5 w-3.5 transition-opacity opacity-50",
  ...
  const failedOrDeleted = useMemo(
    () => nodes.filter((n) => ["FAILED", "DELETED"].includes(String(n.status))),
    [nodes]
  );

  const clearFailedDeleted = async () => {
    if (failedOrDeleted.length === 0) return;
    setBulkDeleting(true);
    // optimistic prune
    setNodes((prev) => prev.filter((n) => !["FAILED", "DELETED"].includes(String(n.status))));
    try {
      await Promise.allSettled(
        failedOrDeleted.map((n) => fetch(`/v1/cluster/${n.id}`, { method: "DELETE" }))
      );
    } finally {
              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="space-y-4">
                  <ol className="list-none space-y-3">
                    ...
                    <li>
                      📦 &nbsp;Run{" "}
                      <code className="bg-gray-100 px-1 py-0.5 rounded">pip install burla</code>
                    </li>
                    <li>
                      🔑 &nbsp;Run{" "}
                      <code className="bg-gray-100 px-1 py-0.5 rounded">burla login</code>
                    </li>
                    <li>
                      🚀 &nbsp;Run some code:
                      <br />
                      <div className="relative mt-3 inline-block w-fit max-w-full">
                        ...
                        <pre className="bg-gray-50 border rounded p-3 overflow-x-auto text-sm font-mono pr-14 w-fit max-w-full">
                          <code>
                            <span className="text-blue-700">from</span> burla{" "}
                            <span className="text-blue-700">import</span> remote_parallel_map
                            <br />
                            <br />
                            ...
                            {"{"}x{"}"}<span className="text-red-700">"</span>)
                            <br />
                            <br />
                            remote_parallel_map(
                ...
                <TableRow>
                  ...
                  <TableHead className="w-24 pl-6 pr-4 py-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 h-6 leading-none hover:text-primary"
                    >
                      Status
                  ...
                  <TableHead className="w-8 pl-6 pr-2 py-2 text-right">
                    {failedOrDeleted.length > 0 ? (
                      <button
                        type="button"
                        onClick={clearFailedDeleted}
                        disabled={bulkDeleting}
                        aria-label="Clear failed or deleted nodes"
                        className={cn(
                          "inline-flex items-center justify-center rounded px-2 py-1 text-sm font-medium border leading-none h-7",
                          bulkDeleting
                            ? "cursor-not-allowed text-gray-400 border-gray-200 bg-gray-50"
                            : "text-red-700 border-red-200 hover:bg-red-50"
                        )}
                        title="Clear failed/deleted"
                      >
                        x&nbsp;clear
                      </button>
                    ) : null}
                  ...
                  <React.Fragment key={node.id}>
                    ...
                    >
                      ...
                      <TableCell className="w-24 pl-6 pr-4 py-2">
                        <div className="flex items-center space-x-2">
                          <div className={getStatusClass(node.status)} />
                          <span className={cn("text-sm capitalize", node.status)}>{node.status}</span>
                        </div>
                      </TableCell>
                      <TableCell className="w-48 pl-6 pr-4 py-2 whitespace-nowrap">
                        {node.name}
                      ...
                      <TableCell className="w-8 pl-6 pr-2 py-2 text-center">
                        {(String(node.status) === "FAILED" || String(node.status) === "DELETED") && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              deleteNode(node.id);
                    ...
                    {expandedNodeId === node.id && (
                      ...
                      >
                        <TableCell colSpan={7} className="p-0">
                          <div
                            className={cn("overflow-y-auto transition-all duration-300", {
                              "max-h-0": expandedNodeId !== node.id,
                              "h-[400px] resize-y py-2 px-4": expandedNodeId === node.id,
                            })}
                          >
                            {logsLoading[node.id] ? (
                              <div className="flex flex-col items-center justify-center h-40 w-full text-gray-500">
