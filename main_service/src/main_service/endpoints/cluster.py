...
#                         "started_booting_at": doc_data.get("started_booting_at")
#                     }
...
def _to_epoch_ms(ts):
    if ts is None:
        return None
    if isinstance(ts, (int, float)):
        # Firestore value is usually seconds (float) in your DB
...
def get_deleted_recent_paginated(
    ...
    for status in statuses:
        q = DB.collection("nodes").where(filter=FieldFilter("status", "==", status))
        ...
        for doc in q.stream():
            ...
            # filter window uses whichever timestamp exists
            sort_ms = deleted_ms or started_ms
            if not sort_ms:
                continue

            sort_dt = datetime.utcfromtimestamp(sort_ms / 1000.0)
            if sort_dt < seven_days_ago:
                continue
            ...
            results.append(
                {
                    ...
                    "gpus": data.get("num_gpus"),
                    "memory": data.get("memory"),
                    # for UI sorting: always use started_booting_at primarily
                    "started_booting_at": started_ms,
                    # keep existing field for UI display / fallback
                    "deletedAt": sort_ms,
                }
            )

    # newest first by started_booting_at, fallback to deletedAt
    results.sort(
        key=lambda x: (x.get("started_booting_at") or 0, x.get("deletedAt") or 0),
        reverse=True,
    )
