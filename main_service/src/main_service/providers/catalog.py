...
AWS CPU nodes are m7i.<size> (also 4 GiB RAM per vCPU), so capacity math is
...
# (pricing isn't defined for it). m7i.12xlarge omitted for symmetry.
...
def machine_spec(machine_type: str) -> dict:
    ...
    if machine_type.startswith("m7i.") and machine_type.split(".")[-1] in _M7I_SIZE_TO_CPUS:
...
def is_packable_cpu_machine(machine_type: str) -> bool:
    ...
    return machine_type.startswith("n4-standard-") or machine_type.startswith("m7i.")
