...
local-dev-cluster:
	set -e; \
	echo "Recording current time to determine if cluster should autostart..."
	printf "$$(date +%s)" > main_service/.local_cluster_last_started_at.txt; \
	docker network create local-burla-cluster || true; \
	docker run --rm -it \
		--name main_service \
		--network local-burla-cluster \
