...
- `<project-id>.relay.burla.dev` -> head VM (dashboard + API)
...
2. **Backend**: `POST /v1/clusters/{project_id}/dashboard` now receives an
   ...
   tokens with `GET /v1/clusters/{project_id}/dashboard_url`.)
3. **Let's Encrypt rate limits**: each cluster head requests a cert for
   `<project-id>.relay.burla.dev`. burla.dev is one registered domain, so the
   default 50 certs/week cap applies; request a rate-limit increase before
   this exceeds ~40 new clusters/week.
