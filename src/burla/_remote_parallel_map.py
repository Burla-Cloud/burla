...
# Error from inside the main service that should be passed to the user:
...
def _start_job(
    ...
    if response.status_code == 401:
        raise AuthException()
    elif response.status_code != 200 and response.json().get("error_type"):
        msg = f"{response.json().get('error_type')}\n{response.json().get('message')}"
        raise MainServiceError(msg)
    else:
        response.raise_for_status()
...
def _watch_job(
    ...
    args = (job_doc_ref, stop_event, spinner)
